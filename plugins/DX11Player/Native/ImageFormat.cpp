...
ImageFormat::ImageFormat(const std::string &imageFile) :
  ...
  if (extension == ".dds") {
    processDDS(imageFile);
    //return;
  }
  else if (extension == ".tga") {
    processTGA(imageFile);
    //return;
  }
  else if (extension == ".dpx") {
    processDPX(imageFile);
    //return;
  } else {
  ...
  /*std::stringstream ss;
  ss << "loading " << m_ImageFiles.size() << " images from " << directory << " "
  << out_w << "x" << h << " " << pixel_format << " with " << bytes_data << "
  bytes per file and " << data_offset << " data_offset, row_pitch: " <<
  row_pitch << " input tex format " << in_format << " and output tex format " <<
  out_format << std::endl; OutputDebugStringA( ss.str().c_str() ); */
}

void ImageFormat::processDPX(const std::string & imageFile)
{
	dpx::Header header;
	InStream stream;
	...
	std::stringstream str;
	str << "dpx with " << header.numberOfElements
      << " elements and format "
      << dpx_descriptor
      << " "
      << (int)header.BitDepth(0)
      << "bits packed with "
      << header.ImagePacking(0)
      << std::endl;
	...
	case dpx::Descriptor::kRGB:
		...
		case 8:
			if (header.Width() * 3 % 4) {
				OutputDebugStringA(
					("8bit dpx rgb on r8 " + std::to_string(header.Width()) + "\n")
					.c_str());
				in_format = DXGI_FORMAT_R8_UNORM;
		...
		case 10:
			if (header.ImagePacking(0) == 1) {
				...
				throw std::exception(
					"RGB 10bits without packing to 10/10/10/2 not supported");
			}
			break;
		case 16:
...
void ImageFormat::processTGA(const std::string & imageFile)
{
	TGA_HEADER header;
	...
	if (header.datatypecode != 2) {
		std::stringstream str;
		str << "tga format " << (int)header.datatypecode
...
void ImageFormat::processDDS(const std::string & imageFile) {
	DirectX::TexMetadata mdata;
	DirectX::GetMetadataFromDDSFile(imageFile.c_str(),
	DirectX::DDS_FLAGS_NONE, mdata);
	in_format = mdata.format;
	// TODO: check out_format
	out_format = mdata.format;
	// out_format = DXGI_FORMAT_R8G8B8A8_UNORM;
	size_t blockSize;
	if (in_format == DXGI_FORMAT_BC1_UNORM ||
      in_format == DXGI_FORMAT_BC1_UNORM_SRGB ||
      in_format == DXGI_FORMAT_BC4_UNORM ||
      in_format == DXGI_FORMAT_BC4_SNORM) {
		h = NextMultiple(mdata.height, 4);
		w = NextMultiple(mdata.width, 4);
		blockSize = 8;
		bytes_per_pixel_in = 2;
		out_w = w;
	} else if (in_format == DXGI_FORMAT_BC2_UNORM ||
         		 in_format == DXGI_FORMAT_BC2_UNORM_SRGB ||
         		 in_format == DXGI_FORMAT_BC3_UNORM ||
         		 in_format == DXGI_FORMAT_BC3_UNORM_SRGB ||
         		 in_format == DXGI_FORMAT_BC5_UNORM ||
         		 in_format == DXGI_FORMAT_BC5_SNORM ||
         		 in_format == DXGI_FORMAT_BC6H_SF16 ||
         		 in_format == DXGI_FORMAT_BC6H_UF16 ||
         		 in_format == DXGI_FORMAT_BC7_UNORM ||
         		 in_format == DXGI_FORMAT_BC7_UNORM_SRGB) {
		h = NextMultiple(mdata.height, 4);
		w = NextMultiple(mdata.width, 4);
		blockSize = 16;
		bytes_per_pixel_in = 4;
	...
	OutputDebugStringA(("width: " + std::to_string(mdata.width) + " pitch " +
		std::to_string(mdata.rowPitch) + "\n")
		.c_str());
}
