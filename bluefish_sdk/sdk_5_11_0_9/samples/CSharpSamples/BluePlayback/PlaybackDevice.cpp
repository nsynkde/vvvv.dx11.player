...
void CPlaybackDevice::Upload(BYTE* pBuffer)
{
	...
	/*wchar_t DebugString[256];
	swprintf_s(DebugString, 256, L"upload %d siye, %d active buffer",m_Buffersize,m_ActiveBuffer);
	OutputDebugString(DebugString);*/
...
void CPlaybackDevice::RenderNext()
{
	...
	if(ready[m_FrontBuffer]){
		...
		//ResetEvent(OverlapChA[m_FrontBuffer].hEvent);
